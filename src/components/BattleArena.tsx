      ...
      <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
        ...
        <div
          className={`relative w-full max-w-sm rounded-lg border-2 p-4 shadow-2xl ${
            isCritical ? "bg-[#ff6b6b]/20 border-[#ff6b6b]" : "bg-[#242424] border-[#ffc700]/50"
          }`}
        >
          <div className="flex items-start gap-3">
            <AlertCircle className={`w-5 h-5 mt-0.5 flex-shrink-0 ${isCritical ? "text-[#ff6b6b]" : "text-[#ffc700]"}`} />
            <div className="flex-1">
              <h3 className="font-bold text-sm text-white mb-1">
                {isCritical ? "Critical Violation" : "Contest Violation Detected"}
              </h3>
              <p className="text-xs text-[#cccccc] mb-3">{violation.details}</p>
              <div className="flex items-center gap-2 mb-3">
                <div className="flex-1 h-1.5 bg-black/40 rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all ${isCritical ? "bg-[#ff6b6b]" : "bg-[#ffc700]"}`}
...
const FlaggedBanner = memo(({ flagReason }: { flagReason?: string }) => (
  <div className="absolute inset-0 z-[200] bg-[#ff6b6b]/20 backdrop-blur-sm flex items-center justify-center pointer-events-none">
    <div className="bg-[#ff6b6b]/30 border-2 border-[#ff6b6b] rounded-lg p-6 text-center max-w-md">
      ...
      <p className="text-xs text-[#cccccc]">This incident has been logged and reported to administrators.</p>
    </div>
  </div>
));
...
const DifficultyBadge = memo(({ difficulty }: { difficulty: string }) => {
  const colorMap: Record<string, string> = {
    ...
    medium: "bg-[#ffc700]/20 text-[#ffc700] border-[#ffc700]/30",
...
export default memo(function BattleArena({
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full flex flex-col bg-[#0a0a0a] font-['Space_Grotesk'] overflow-hidden">
      {/* ── Top Status Bar ────────────────────────────── */}
      <header className="h-10 bg-[#1a1a1a] border-b border-[#999999]/20 flex items-center px-4 gap-4 shrink-0">
        <div className="flex items-center gap-3">
          <Shield className="w-4 h-4 text-[#ffc700]" />
          <span className="text-[10px] font-bold text-[#ffc700] uppercase tracking-[0.08em]">
        ...
        <div className="flex-1 flex items-center gap-4 mx-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-[#cccccc]">15:42</span>
          </div>
          <div className="h-1 flex-1 bg-[#242424] rounded-full overflow-hidden">
            <div className="h-full w-3/4 bg-gradient-to-r from-[#ffc700] to-[#ffd700]" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 bg-[#242424] rounded border border-[#555555]">
            <span className="text-[10px] text-[#999999]">Participants:</span>
            <span className="text-[10px] font-bold text-[#ffc700]">{players.length}</span>
          </div>

          {!isPractice && violationCount > 0 && (
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded border ${
              violationCount >= 2 ? "bg-[#ff6b6b]/20 border-[#ff6b6b] animate-pulse" : "bg-[#ffc700]/20 border-[#ffc700]"
            }`}>
      ...
      <div className="flex-1 flex overflow-hidden gap-0">
        ...
        {problemVisible && (
          <div className="w-80 shrink-0 overflow-y-auto bg-[#0a0a0a] border-r border-[#999999]/20 px-5 py-4 space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="p-1 text-[#999999] hover:text-[#ffc700] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {problem?.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {problem.tags.map((tag: string) => (
                  <span key={tag} className="text-[10px] px-2 py-1 rounded bg-[#242424] border border-[#555555] text-[#cccccc]">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            {problem?.sampleInput && (
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold text-[#ffc700] uppercase">Input Format</p>
                <pre className="bg-[#1a1a1a] border border-[#555555] rounded px-3 py-2 text-xs text-[#00cc44] overflow-x-auto">
                  {problem.sampleInput}
                </pre>
              </div>
            )}
            ...
            {problem?.sampleOutput && (
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold text-[#ffc700] uppercase">Output Format</p>
                <pre className="bg-[#1a1a1a] border border-[#555555] rounded px-3 py-2 text-xs text-[#66b3ff] overflow-x-auto">
                  {problem.sampleOutput}
                </pre>
              </div>
            )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Editor Header */}
          <div className="h-10 bg-[#1a1a1a] border-b border-[#999999]/20 flex items-center px-3 gap-2 shrink-0">
            <span className="text-[10px] font-bold text-[#ffc700] uppercase tracking-[0.08em]">Editor</span>

            <div className="relative" ref={dropdownRef}>
              <button
                onClick={() => setShowDropdown(!showDropdown)}
                className="flex items-center gap-1.5 px-2 py-0.5 text-[10px] bg-[#242424] hover:bg-[#2e2e2e] border border-[#555555] rounded text-[#ffffff] transition-colors"
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 mt-1 bg-[#1a1a1a] border border-[#555555] rounded shadow-lg z-50 min-w-[130px] overflow-hidden">
                  {LANGUAGES.map((lang) => (
                    <button
                      key={lang.value}
            ...
            <button
              ...
              disabled={isRunning || isSubmitting || battleFinished}
              className="flex items-center gap-1 px-2 py-0.5 text-[10px] bg-[#ffc700] hover:bg-[#ffd700] disabled:opacity-50 text-black font-bold rounded transition-colors"
            >
          ...
          {/* Terminal */}
          <div className="h-40 overflow-y-auto bg-[#0a0a0a] border-t border-[#999999]/20 px-4 py-3">
            <div className="px-4 py-2 border-b border-[#999999]/20 flex items-center gap-2 mb-3">
              <span className="text-[10px] font-bold text-[#ffc700] uppercase">Terminal Output</span>
              {executionResult?.executionTime && <span className="text-[9px] text-[#999999] ml-auto">{executionResult.executionTime}ms</span>}
            </div>

            {isRunning || isSubmitting ? (
              <div className="flex items-center gap-2 text-[#cccccc]">
                <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#ffc700]" />
                ...
                {executionResult.success ? (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <pre className="bg-[#1a1a1a] border border-[#00cc44]/20 rounded p-2 text-xs text-[#00cc44] overflow-auto max-h-24">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <pre className="bg-[#1a1a1a] border border-[#ff6b6b]/20 rounded p-2 text-xs text-[#ff6b6b] overflow-auto max-h-24">
              ...
              <p className="text-[#999999]">Output will appear here…</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* ── Status Bar ────────────────────────────────── */}
      <footer className="h-8 bg-gradient-to-r from-[#ffc700] to-[#ffd700] border-t border-[#ffc700]/20 flex items-center px-4 text-[10px] text-black font-bold uppercase tracking-wider shrink-0">
      ...
      {battleFinished && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#1a1a1a] border border-[#ffc700]/20 rounded-lg overflow-hidden shadow-2xl">
            <div className="bg-gradient-to-r from-[#ffc700] to-[#ffd700] px-6 py-4 flex items-center gap-3">
            ...
            <div className="p-4 space-y-2">
              {players.map((p) => (
                <div key={p.id} className={`flex items-center gap-3 px-3 py-2 rounded border ${
                  p.id === user.id
                    ? "bg-[#ffc700]/10 border-[#ffc700]/20"
                    : "bg-[#242424] border-[#555555]"
                }`}>
                  {p.isSolved ? (
                  ...
                  <span className="flex-1 text-sm text-[#ffffff]">
                    {p.username}
                    {p.id === user.id && <span className="text-[#999999] ml-1">(you)</span>}
                  </span>
                  <span className="text-xs text-[#999999]">{Math.floor(p.progress)}% · {p.currentScore}pts</span>
                </div>
              ))}
            </div>
            ...
            <div className="px-4 pb-4">
              <button
                ...
                className="w-full py-2.5 bg-[#ffc700] hover:bg-[#ffd700] text-black font-bold text-sm rounded transition-colors"
              >
                Return to Lobby
              </button>
            </div>
